...
def update_exp():
    ...
    with open("/share2/zyu9/Datasets/SLR//dump/Hall2007.pickle","rb") as handle:
    ...
    with open("/share2/zyu9/Datasets/SLR//dump/Hall2010.pickle","rb") as handle:
        csr_mat2 = pickle.load(handle)
        labels2 = pickle.load(handle)
        vocab2 = pickle.load(handle)
    # with open("../dump/ieee.pickle","rb") as handle:
    with open("/share2/zyu9/Datasets/SLR//dump/ieee.pickle","rb") as handle:
    ...
    if rank == 0:
        for i in range(proc_num-1):
            ...
            results.extend(tmp)
            print("rand %d received" %i)
        with open("../dump/repeat_update.pickle","w") as handle:
            pickle.dump(results, handle)
    else:
        comm.send(results,dest=0)
        print("rank %d sent" %rank)
