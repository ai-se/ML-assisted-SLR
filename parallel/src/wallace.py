...
class Wallace(object):
    ...
    def create(self, filename):
        ...
        self.last_neg = 0
        self.est2 = []
        self.record_est={"x": [], "est": [], "sigmoid": [], "Horvitz_Thompson": []}
    ...
    def estimate_curve(self, clf):
        ...
        lamb = sum(pre)/self.step
        z = pre/lamb
        sample=[]
        while sample==[]:
            sample = [x for x in self.pool if random.random()<z[x]]

        est1=len(poses)+sum(pre[self.pool])
        try:
            est_tmp = sum((sum(self.est)/self.step/self.est)[self.last_sample]*[1 if self.body['code'][s]=='yes' else 0 for s in self.last_sample])/sum((sum(self.est)/self.step/self.est)[self.last_sample])
            self.est2.append(est_tmp)
            est2=sum(self.est2)/len(self.est2)*(len(self.body["code"]) - self.last_pos - self.last_neg)
        except:
            self.est2=[]
            est2=est1
    ...
    def train(self, pne=True):
        ...
        return uncertain_id, self.est[uncertain_id], self.last_sample, self.est[self.last_sample]
    ...
    def get_rest(self):
        rest = [x for x in xrange(len(self.body['label'])) if
                self.body['label'][x] == 'yes' and self.body['code'][x] != 'yes']
        rests = {}
        # fields = ["Document Title", "Abstract", "Year", "PDF Link"]
